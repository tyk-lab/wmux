...
- 缺少 Win32/GUI/桌面自动化通道属于“验证能力受限”，不是实现异常或协议故障。最多执行一轮与失败路线明显不同的替代测试、基础测试或静态证据；核心交互仍需观察或替代路线仍无新证据时，如实上报项目 AI，由项目 AI请求用户选择人工验收、暂缓验证、跳过当前验证并后续重排，或保持暂停。不得重复原自动化路径、重复派发同义任务，或把无法自动化写成执行失败。
