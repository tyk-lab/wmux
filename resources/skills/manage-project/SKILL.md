...
Every new work item has a control-plane-owned project-baseline gate. Do not include or forge an approved `baseline` in task JSON. Before any write, dependency install, build/test, device action, or permission confirmation, the supervisor must send a `[项目基线调查]` instruction. The task AI performs one bounded read-only inspection and returns `[项目基线报告]` covering the current git/uncommitted state, relevant structure and entry/call paths, existing build/test conventions, known failures or missing pieces, expected write paths, shared resources, recommended execution mode, and the next safe action. The supervisor reviews the report and approves it only with `[批准项目基线]` plus structured `--workspace-version`, `--evidence`, and `--stage-plan-file`; the control layer blocks implementation and completion until this succeeds. This atomic approval must omit result fields such as `--changed-files`, `--test-command`, `--test-result`, `--full-suite`, and `--retry`; future write paths and validation commands belong in the stage plan's `expectedPaths` and `targetedValidation`. `expectedPaths` lists only paths that the task AI will intentionally create or modify inside the contract scope, not compiler/build-generated binaries, caches, or temporary outputs unless they are explicitly authorized deliverables. If the first report lacks one fact that would materially change the execution route, the supervisor may request one targeted read-only supplement; it must not repeat the full investigation, and the control plane rejects a third investigation round. After that, approve from the available evidence or report one concrete blocker with a pause/replan recommendation. Reuse an approved baseline only for the same work item and requirements version. Requirement rebinding, contract changes, or application recovery resets it and requires a fresh current-worktree check.

If the control plane reports `supervisor.decision-error-loop`, the dedicated supervisor has repeated the same invalid decision twice and is paused without discarding its baseline or stage-plan draft. Inspect the recorded error and current contract, update the existing work item's direction or version when necessary, then resume that same work item with `supervise`. Do not create a replacement work item merely to escape the protocol guard.
