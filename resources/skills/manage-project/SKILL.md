...
9. Every dispatched work item carries a control-plane `[项目执行身份｜控制层已绑定]` block containing the project, main-goal, work-item, requirements, and authorization versions. That block is the current runtime identity. On project creation, recovery, or work-item switching, never search for, wait for, or reconstruct an old lane/terminal/conversation identity. A domain-specific identity, profile, candidate mapping, or registration artifact that can be created or derived inside the current project scope is ordinary setup work, not a reason to ask the user or debate the same missing fact repeatedly.
