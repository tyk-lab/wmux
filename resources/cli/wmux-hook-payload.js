...
exports.parseWmuxHookPayload = parseWmuxHookPayload;
exports.stableWmuxHookId = stableWmuxHookId;
...
function parseWmuxHookPayload(stdinData, fallbackEvent = '') {
    let data = {};
    try {
    ...
    return {
        ...
        message: compact(lastAssistantMessage
            || data.message
        ...
        turnId: compact(data.wmux_turn_id || data.turn_id || data.turnId || data.prompt_id || data.promptId),
    };
}
...
function stableWmuxHookId(options) {
    if (!TURN_BOUNDARY_EVENTS.has(options.event) || !options.turnId)
        return undefined;
