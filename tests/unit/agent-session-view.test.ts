...
import { agentSessionsForWorkspace } from '../../src/renderer/store/agent-session-view';
...
describe('agentSessionsForWorkspace', () => {
  it('returns no sessions when neither hooks nor explicit activity saw an agent', () => {
  ...
  it('a fresh hook event makes the surface a working session with its tool', () => {
    ...
    const out = agentSessionsForWorkspace(tree, {}, { 'surf-a': hook('Edit', NOW - 1000) }, NOW);
  ...
  it('a Stop-zeroed hook entry keeps the session listed but idle', () => {
    const tree = leaf('pane-1', [{ id: 'surf-a' }]);
    const out = agentSessionsForWorkspace(tree, {}, { 'surf-a': hook('Bash', 0) }, NOW);
  ...
  it('tracks two sessions independently — one working, one idle', () => {
    ...
    const out = agentSessionsForWorkspace(tree, {}, {
...
describe('declared agent state precedence', () => {
  ...
  it('without a declared state the same stale surface reads idle — the old bug', () => {
    const tree = leaf('pane-1', [{ id: 'surf-a' }]);
    const out = agentSessionsForWorkspace(tree, {}, { 'surf-a': hook('Bash', NOW - 60_000) }, NOW);
