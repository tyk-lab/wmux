...
1. 在与机器人的单聊发送：`帮助`。
2. 点击**添加终端任务**：普通终端可填写任务名称、首条任务，并选择 Codex、Kimi 或 Grok（默认 Codex）。wmux 会在 `桌面\wmux任务\<任务名称>-时间戳` 创建目录，以任务名新建独立工作区，启动对应的直连 AI 终端，并在终端就绪后发送首条任务。创建项目管理终端前须已有被监督任务终端：只有一条监督通道时直接以它为锚点，多条通道时先选择锚点；wmux 会在锚点所在工作区和窗格中新建并选中唯一的 Grok“项目管理终端”标签，工作目录固定为 `K:\sync_code\Link_Folder_108952\toolbox\build\windows\x64\runner\Release`，并首先发送 `/inspect-project-progress`。再次创建只会切换到已有标签。固定目录或技能不存在时会明确报错。
3. 点击**启动监督**：新建的直连终端和其他未监督终端都会出现在列表中；选择终端并填写停止条件即可添加监督。标有“已停止，可重新监督”的终端也可重新启动监督。
4. 点击**终端控制**：选择任务终端后，同一张卡会显示终端名称、空闲/执行中/等待人工/未知状态、抓取时间和最近核心文本，并提供多行输入、发送和刷新。刷新会保留未发送草稿且不会把草稿提交到终端；若终端仍在执行，发送仍需二次确认。输出会合并软换行、折叠重复刷新帧和空行，并保留最新约 18 行、最多约 1500 字。由于可能包含敏感信息，该入口只在白名单用户单聊中显示。
