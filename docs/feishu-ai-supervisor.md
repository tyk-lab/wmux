...
- 审计群为每个任务终端维护一张可更新的当前状态卡，集中显示当前任务、任务阶段、监督状态、最近情况、下一步和待人工事项；失败、阻塞、完成和待人工等关键变化会额外提醒，终端屏幕全文不会外发。
...
默认请在与机器人的单聊中发送“**帮助**”（也兼容 `WMUX HELP` 或“wmux帮助”）。机器人会显示**添加终端任务、启动监督、发送任务、管理监督、刷新状态**菜单。其他操作可通过卡片选择终端、填写表单，无需复制 `surfaceId` 或输入长命令。终端当前状态卡和关键提醒发送到 `WMUX_FEISHU_CHAT_ID` 指定群，待决卡片和决策结果只发送到白名单用户单聊；群里仅显示“等待人工”等通用状态。推荐设置 `WMUX_FEISHU_DECISION_CHAT_ID`，使 wmux 每次启动后都能直接推送人工决策；未设置时使用最近联系机器人的白名单单聊，并在首次有效单聊前暂存待决消息。仅在设置了 `WMUX_FEISHU_CONTROL_CHAT_ID` 后，才可在该独立控制群发送高级非决策文本命令。
...
5. 点击**管理监督**：集中暂停、继续或停止全部监督，也可单独控制监督通道，并查看对应任务终端的当前状态。
6. 点击**刷新状态**：重新读取当前监督状态和可用终端。
