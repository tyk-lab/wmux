...
4. 点击**启动监督**：新建的直连终端和其他未监督终端都会出现在列表中；选择终端，填写停止条件，并至少填写任务目标或选择该终端项目目录内的计划文件。标有“已停止，可重新监督”的旧协议终端需要新建任务终端后再按当前协议启动。
...
当监督 AI 使用 `needs-human` 提交重要建议时，wmux 会向 `WMUX_FEISHU_DECISION_CHAT_ID` 配置的单聊发送决策卡片；未配置时使用最近联系机器人的白名单用户单聊，且在首次有效单聊前暂存卡片。决策卡不会回退发送到审计群。卡片会显示 AI 原建议，以及发送卡片时读取到的任务终端最新可见界面原文；终端内容只进入白名单用户的决策单聊，不发送到审计群。若 AI 提供至少两个“方案 A / 方案 B”等明确选项，卡片会提供必选下拉框：

审计群中的待决提醒用于让成员理解上下文，只展示脱敏后的“当前任务目标 / 需要用户决定 / 为什么需要决定”。具体方案、AI 推荐、终端原文、用户最终选择和处理按钮均留在白名单用户单聊中。

- **确认并采用 AI 方案**：先从下拉框选择一个备选方案，再将选择交回对应 AI 监督；多方案时未选择会拒绝提交并保留决策卡。AI 监督会读取任务终端最新状态，将所选方案整理为完整、可执行的下一步，再通过监督裁决通道发送到任务终端。AI 未提供多个明确方案时，按钮改为“采用 AI 当前方案”，无需选择。
