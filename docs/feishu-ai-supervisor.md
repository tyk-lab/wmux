...
- 白名单用户可在与机器人的单聊中创建 Codex 直连终端、列出已有终端、开启或停止 AI 监督，并向指定终端发送任务；操作结果返回该单聊。
...
默认请在与机器人的单聊中发送“**帮助**”（也兼容 `WMUX HELP` 或“wmux帮助”）。机器人会显示**添加终端任务、启动监督、发送任务、管理监督、刷新状态**菜单。其他操作可通过卡片选择终端、填写表单，无需复制 `surfaceId` 或输入长命令。普通监督审计摘要发送到 `WMUX_FEISHU_CHAT_ID` 指定群，待决卡片和决策结果只发送到白名单用户单聊。推荐设置 `WMUX_FEISHU_DECISION_CHAT_ID`，使 wmux 每次启动后都能直接推送人工决策；未设置时使用最近联系机器人的白名单单聊，并在首次有效单聊前暂存待决消息。仅在设置了 `WMUX_FEISHU_CONTROL_CHAT_ID` 后，才可在该独立控制群发送高级非决策文本命令。
...
1. 在与机器人的单聊发送：`帮助`。
2. 点击**添加终端任务**：填写任务名称和首条任务。wmux 会在 `桌面\wmux任务\<任务名称>-时间戳` 创建目录，以任务名新建独立工作区，启动一个 `Codex直连` 终端，并在 Codex 就绪后发送首条任务。该终端默认不加入监督，关闭终端或停止监督都不会删除任务目录。
