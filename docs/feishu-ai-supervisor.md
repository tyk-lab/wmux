...
默认请在与机器人的单聊中发送“**帮助**”（也兼容 `WMUX HELP` 或“wmux帮助”）。机器人会显示**添加终端任务、终端控制、关闭终端、启动监督、管理监督、刷新状态**菜单。其他操作可通过卡片选择终端、填写表单，无需复制 `surfaceId` 或输入长命令。终端当前状态卡和关键提醒发送到 `WMUX_FEISHU_CHAT_ID` 指定群，待决卡片、决策结果和包含终端核心文本的控制卡只发送到白名单用户单聊；群里仅显示“等待人工”等通用状态。推荐设置 `WMUX_FEISHU_DECISION_CHAT_ID`，使 wmux 每次启动后都能直接推送人工决策；未设置时使用最近联系机器人的白名单单聊，并在首次有效单聊前暂存待决消息。仅在设置了 `WMUX_FEISHU_CONTROL_CHAT_ID` 后，才可在该独立控制群发送高级非决策文本命令。
...
1. 在与机器人的单聊发送：`帮助`。
2. 点击**添加终端任务**：普通终端可填写任务名称、首条任务，并选择 Codex、Kimi 或 Grok（默认 Codex）。不选择“终端路径”时，wmux 仍会在 `桌面\wmux任务\<任务名称>-时间戳` 创建目录；也可从当前任务终端的实时工作目录中选择一个去重后的路径，让新终端直接在该目录启动。随后 wmux 会以任务名新建独立工作区，并在终端就绪后发送首条任务。创建项目管理终端前须已有被监督任务终端：只有一条监督通道时直接以它为锚点，多条通道时先选择锚点；wmux 会在锚点所在工作区和窗格中新建并选中唯一的 Grok“项目管理终端”标签，工作目录固定为 `K:\sync_code\Link_Folder_108952\toolbox\build\windows\x64\runner\Release`，并首先发送 `/inspect-project-progress`。再次创建只会切换到已有标签。固定目录或技能不存在时会明确报错。
3. 点击**启动监督**：新建的直连终端和其他未监督终端都会出现在列表中；选择终端并填写停止条件即可添加监督。标有“已停止，可重新监督”的终端也可重新启动监督。
4. 点击**终端控制**：选择任务终端后，同一张卡会显示终端名称、空闲/执行中/等待人工/未知状态、抓取时间，并优先把最近一轮内容区分为“你的提问”和“Agent 回复”。Codex、Grok、Kimi 等 AI 终端会过滤启动信息、思考过程、工具调用、搜索状态、hooks、耗时、隐私提示和输入框；Codex 仍在执行时仅显示“回复生成中”，不会把中间过程当成最终回复。无法可靠识别对话结构时回退到最近核心文本。卡片同时提供多行输入、发送和刷新；刷新会保留未发送草稿且不会把草稿提交到终端，若终端仍在执行，发送仍需二次确认。由于可能包含敏感信息，该入口只在白名单用户单聊中显示。
5. 点击**关闭终端**：先选择任务终端，再查看当前状态和影响并二次确认。专属监督 AI 终端不会出现在可关闭列表中；若任务终端正在被监督，确认后会先写入审计、解除监督绑定并关闭对应专属监督终端，再复用桌面端关闭逻辑回收任务终端 PTY。关闭最后一个标签时对应工作区也会关闭，但任务目录和历史审计记录始终保留。
6. 点击**管理监督**：集中暂停、继续或停止全部监督，也可单独控制监督通道，并查看对应任务终端的当前状态。
7. 点击**刷新状态**：重新读取当前监督状态和可用终端。
...
- **直接发送用户输入**：在卡片中填写具体决策信息后，wmux 将内容直接提交到对应任务终端，并记录为人工裁决，不经过 AI 监督整理。
- **暂停监督**：保留当前会话、监督终端和待决项；继续监督后仍可使用原决策卡。
- **停止监督**：停止当前监督会话。

直接发送时用户输入优先于 AI 监督建议；若任务终端已有未提交草稿或输入状态不可用，wmux 会保留待决项并提示用户处理后重试，避免覆盖或拼接现有输入。
...
2. 点击**采用 AI 方案**。
3. 确认单聊卡片更新为“人工决策已处理”；对应 AI 监督收到选择并结合最新终端状态整理信息，随后工作终端收到整理后的任务指令。决策内容不会发送到审计群。

也可在同一张卡片填写“用户决策信息”，点击**直接发送用户输入**，确认任务终端直接收到填写内容，且对应待决项结束。
