...
- 白名单用户可在与机器人的单聊中创建 Codex、Kimi 或 Grok 直连终端、列出已有终端、查看和刷新核心输出、发送任务、安全关闭任务终端，以及开启或停止 AI 监督；操作结果返回该单聊。
- 日常控制无需记忆命令：发送“帮助”即可打开菜单，按卡片创建终端、选择终端并填写任务或停止条件。
- 审计群为每个任务终端维护一张可更新的当前状态卡，集中显示当前任务、任务阶段、监督状态、最近情况、下一步和待人工事项；等待人工决策时，群卡只说明当前任务目标、需要用户决定什么以及为什么必须由用户决定。失败、阻塞、完成和待人工等关键变化会额外提醒，终端屏幕全文不会外发。
- 当监督 AI 提交“需要人工决定”的建议时，配置的决策单聊（未配置时为最近联系机器人的白名单用户单聊）会收到带“采用方案 / 提交用户决策 / 暂停监督 / 停止监督”操作的飞书卡片；决定先交回监督 AI 整理，不直送任务终端。
- 在飞书点击卡片或发送固定命令后，结果同步回 wmux；远程操作会写入项目的本地审计记录。
- 仅指定群和 Open ID 白名单内的成员能操作；发送、查看和关闭只接受 `LIST` 返回的当前任务终端，专属监督 AI 终端不会作为这些入口的目标。

## 推荐使用方式

```text
白名单用户与机器人单聊  → LIST / START / SEND / STOP / DECIDE、审批卡片与决策结果
审计群（WMUX_FEISHU_CHAT_ID） → 每终端一张脱敏状态卡 + 关键变化提醒，不接收控制命令
...
默认请在与机器人的单聊中发送“**帮助**”（也兼容 `WMUX HELP` 或“wmux帮助”）。机器人会显示**添加终端任务、创建特别终端、终端控制、关闭终端、启动监督、管理监督、刷新状态**菜单。其他操作可通过卡片选择终端、填写表单，无需复制 `surfaceId` 或输入长命令。终端当前状态卡和关键提醒发送到 `WMUX_FEISHU_CHAT_ID` 指定群，待决卡片、决策结果和包含终端核心文本的控制卡只发送到白名单用户单聊；群里仅显示“等待人工”等通用状态。推荐设置 `WMUX_FEISHU_DECISION_CHAT_ID`，使 wmux 每次启动后都能直接推送人工决策；未设置时使用最近联系机器人的白名单单聊，并在首次有效单聊前暂存待决消息。仅在设置了 `WMUX_FEISHU_CONTROL_CHAT_ID` 后，才可在该独立控制群发送高级非决策文本命令。

人工决策卡提供“采用方案 / 提交用户决策 / 暂停监督 / 停止监督”。采用或补充的决定会先交回监督 AI，由它结合最新证据更新成果计划并重新派发；暂停只保留当前会话和待决项。若用户亲自在对应任务终端发送信息，该输入优先生效，旧待决卡同步取消，监督保持运行。

### 日常卡片操作（推荐）

1. 在与机器人的单聊发送：`帮助`。
2. 点击**添加终端任务**：普通终端可填写任务名称、首条任务，并选择 Codex、Kimi 或 Grok（默认 Codex）。不选择“终端路径”时，wmux 仍会在 `桌面\wmux任务\<任务名称>-时间戳` 创建目录；也可从当前任务终端的实时工作目录中选择一个去重后的路径，让新终端直接在该目录启动。随后 wmux 会以任务名新建独立工作区，并在终端就绪后发送首条任务。
3. 点击**创建特别终端**：创建独立的 Codex“用户记录终端”，工作目录固定为 `K:\sync_code\Link_Folder_108952\toolbox\build\windows\x64\runner\Release`，并在启动时加载 `$user-data-management`。该终端不依赖现有终端，不属于项目 AI 模式，也不按旧“项目终端”标题或目录识别、复用历史会话。再次创建只会切换到由新功能创建并带有专用标记的终端。
4. 点击**启动监督**：新建的直连终端和其他未监督终端都会出现在列表中；选择终端，填写停止条件，并至少填写任务目标或选择该终端项目目录内的计划文件。只识别当前普通监督协议记录，旧协议 lane 不会显示或恢复。
5. 点击**终端控制**：选择任务终端后，同一张卡会显示终端名称、空闲/执行中/等待人工/未知状态、抓取时间，并优先把最近一轮内容区分为“你的提问”和“Agent 回复”。Codex、Grok、Kimi 等 AI 终端会过滤启动信息、思考过程、工具调用、搜索状态、hooks、耗时、隐私提示和输入框；Codex 仍在执行时仅显示“回复生成中”，不会把中间过程当成最终回复。无法可靠识别对话结构时回退到最近核心文本。卡片同时提供多行输入、发送和刷新；刷新会保留未发送草稿且不会把草稿提交到终端，若终端仍在执行，发送仍需二次确认。由于可能包含敏感信息，该入口只在白名单用户单聊中显示。
6. 点击**关闭终端**：先选择任务终端，再查看当前状态和影响并二次确认。专属监督 AI 终端不会出现在可关闭列表中；若任务终端正在被监督，确认后会先写入审计、解除监督绑定并关闭对应专属监督终端，再复用桌面端关闭逻辑回收任务终端 PTY。关闭最后一个标签时对应工作区也会关闭，但任务目录和历史审计记录始终保留。
7. 点击**管理监督**：集中暂停、继续或停止全部监督，也可单独控制监督通道，并查看对应任务终端的当前状态。
8. 点击**刷新状态**：重新读取当前监督状态和可用终端。
...
人工决策通过机器人单聊中的审批卡处理；选择方案或填写用户决策后提交，内容会先交回监督 AI 整理为成果型任务，不会直接转发给被监督终端。
...
4. 发送下方的 `START`，为该已有终端开启 AI 监督。成功后应同时看到单聊成功回复、审计群中新建或更新的终端状态卡，以及 wmux 本地新开的监督终端。
...
- `terminals` 和 `stop_when` 必填；`terminals` 必须来自 `LIST`。`task_goal` 与 `plan_file` 至少填写一项。
- `stop_when_kind` 只能是 `concrete`（可验证条件）或 `direction`（目标方向）；默认 `concrete`。
- `task_description` 可选，仅用于补充说明停止条件，不会作为工作终端的新任务注入。
- `plan_file` 可选，必须是**运行 wmux 的 Windows 电脑上存在、且位于每个目标终端项目目录内的绝对文件路径**；它与 `task_goal`、`stop_when` 一起构成用户规划边界。
...
当监督 AI 使用 `needs-human` 提交重要建议时，wmux 会向 `WMUX_FEISHU_DECISION_CHAT_ID` 配置的单聊发送决策卡片；未配置时使用最近联系机器人的白名单用户单聊，且在首次有效单聊前暂存卡片。决策卡不会回退发送到审计群。卡片会显示 AI 原建议、明确推荐项，以及发送卡片时读取到的任务终端最新可见界面原文；终端内容只进入白名单用户的决策单聊，不发送到审计群。若 AI 提供至少两个“方案 A / 方案 B”等明确选项，卡片会提供下拉框：

审计群中的待决提醒用于让成员理解上下文，只展示脱敏后的“当前任务目标 / 需要用户决定 / 为什么需要决定”。具体方案、AI 推荐、终端原文、用户最终选择和处理按钮均留在白名单用户单聊中。

- **确认并采用 AI 方案**：可直接采用卡片标出的 AI 推荐项，也可先从下拉框改选其他备选方案，再将选择交回对应 AI 监督。AI 监督会读取任务终端最新状态，将所选方案整理为完整、可执行的下一步，再通过监督裁决通道发送到任务终端。AI 未提供多个明确方案时，按钮改为“采用 AI 当前方案”，无需选择。
- **提交用户决策**：在卡片中填写具体决策信息后，wmux 将内容交回对应监督 AI；监督 AI 结合最新证据更新成果计划，再通过结构化任务派发推进。
- **暂停监督**：保留当前会话、监督终端和待决项；继续监督后仍可使用原决策卡。
- **停止监督**：停止当前监督会话。

用户也可以亲自在任务终端输入新任务；该输入优先于监督 AI 建议，控制层只向监督 AI 同步知情，不拦截、不改写，也不会在当前任务回合结束前投递替代任务。
...
`action` 支持 `approve`、`direct`、`pause`、`stop`。AI 提供多个明确方案时，`approve` 必须通过 `selection` 填写当前 AI 建议中列出的完整方案名；只有单一方案或没有结构化备选时才可留空采用 AI 当前建议。`direct` 必须填写 `task`；`task` 不用于其他动作。旧的 `reject` 不再支持。待决项超过 24 小时、已被处理，或监督会话已经停止时，wmux 会拒绝执行旧操作。
...
`direct` 不再直送任务终端；它表示采用卡片中的用户决策文本，并交给监督 AI 重新规划和派发。
...
3. 在单聊确认收到启动回复；再发一次 `LIST`，应显示“监督会话：进行中”和该终端“状态：监督中”。审计群中该终端的状态卡也应显示监督已启动。
...
1. 确认卡片标出的 AI 推荐项；若要改选，在下拉框中选择其他方案。
2. 点击**采用 AI 推荐方案**或**采用所选 AI 方案**。
3. 确认单聊卡片更新为“人工决策已处理”；对应 AI 监督收到选择并结合最新终端状态更新成果计划，随后任务终端只收到当前成果、约束和由监督 AI 给出的本次任务级验收。决策内容不会发送到审计群。

也可在同一张卡片填写“用户决策信息”并提交，确认对应监督 AI 收到该决定、待决项结束，任务终端不会直接收到原始卡片文本。
...
| `START` | 本地创建监督终端，审计群新建或更新该终端的状态卡。 |
| `SEND` | 指定工作终端收到并提交测试任务。 |
| 人工决策 | 审批卡片可选择方案或填写用户决策；提交后由监督 AI 重新规划并派发成果型任务。 |
| `STOP` | 飞书回复停止成功，审计群状态卡显示已停止，`LIST` 显示未启动。 |
...
监督审计事件会按发生顺序聚合到各终端的当前状态卡，普通变化原地更新，不为每个事件新增群消息；失败、阻塞、完成和待人工等关键变化会额外发送提醒。为避免飞书群机器人限流，wmux 会以约 220 毫秒的最小间隔顺序处理卡片。路径、凭据字段和常见令牌会脱敏，终端屏幕全文不会发送。监督 AI 提交 `needs-human` 时，配置的决策单聊（或最近有效的白名单用户单聊）会收到可批准、暂停或停止的审批卡片；审计群只显示通用待人工状态，不包含决策详情，处理结果会同步回 wmux 的当前监督会话。

飞书任务输入和终端关闭受 Open ID 白名单、单聊/独立控制群访问策略、当前终端 ID 校验及重复点击防护保护；由于发送任务会实际向终端输入并提交内容，关闭确认会终止 PTY，请仅向可信用户授予白名单权限。关闭入口不能直接操作专属监督 AI 终端，也不会删除任务目录或审计记录。AI 自主监督仍不会绕过关键风险限制。
