...
- 白名单用户可在与机器人的单聊中创建 Codex、Kimi 或 Grok 直连终端、列出已有终端、开启或停止 AI 监督，并向指定终端发送任务；操作结果返回该单聊。
...
默认请在与机器人的单聊中发送“**帮助**”（也兼容 `WMUX HELP` 或“wmux帮助”）。机器人会显示**添加终端任务、启动监督、发送任务、查看终端界面、管理监督、刷新状态**菜单。其他操作可通过卡片选择终端、填写表单，无需复制 `surfaceId` 或输入长命令。终端当前状态卡和关键提醒发送到 `WMUX_FEISHU_CHAT_ID` 指定群，待决卡片、决策结果和任务终端界面只发送到白名单用户单聊；群里仅显示“等待人工”等通用状态。推荐设置 `WMUX_FEISHU_DECISION_CHAT_ID`，使 wmux 每次启动后都能直接推送人工决策；未设置时使用最近联系机器人的白名单单聊，并在首次有效单聊前暂存待决消息。仅在设置了 `WMUX_FEISHU_CONTROL_CHAT_ID` 后，才可在该独立控制群发送高级非决策文本命令。
...
1. 在与机器人的单聊发送：`帮助`。
2. 点击**添加终端任务**：普通终端可填写任务名称、首条任务，并选择 Codex、Kimi 或 Grok（默认 Codex）。wmux 会在 `桌面\wmux任务\<任务名称>-时间戳` 创建目录，以任务名新建独立工作区，启动对应的直连 AI 终端，并在终端就绪后发送首条任务。也可直接点击**创建项目管理终端**：wmux 会在固定的 `K:\sync_code\Link_Folder_108952\toolbox\build\windows\x64\runner\Release` 目录打开唯一的 Grok“项目管理终端”，并首先发送 `/inspect-project-progress` 读取项目进度技能。该目录或技能不存在时会明确报错。直连终端默认不加入监督，关闭终端或停止监督都不会删除任务目录。
3. 点击**启动监督**：新建的直连终端和其他未监督终端都会出现在列表中；选择终端并填写停止条件即可添加监督。标有“已停止，可重新监督”的终端也可重新启动监督。
4. 点击**发送任务**：直连终端和其他工作终端都会出现在列表中，并显示“空闲 / 执行中 / 等待人工 / 未知”和最近更新时间。选择目标终端并填写任务；若终端仍在执行，需二次确认才会发送，任务正文只在 wmux 内存中临时保留 5 分钟。
5. 点击**查看终端界面**：选择任务终端后读取当前缓冲区末尾 40 行；可在结果卡中刷新当前界面或切换终端。终端原文可能包含敏感信息，因此该入口只在白名单用户单聊中显示，控制群、审计群都不会接收界面内容。
