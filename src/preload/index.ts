...
contextBridge.exposeInMainWorld('wmux', {
  pty: {
    create: (options: { shell: string; cwd: string; env: Record<string, string>; surfaceId?: string; startupCommands?: string[]; supervisorRuntimeIsolationKey?: string; codexSupervisorRuntimeIsolationKey?: string; sshProfileId?: string; cols?: number; rows?: number }) =>
  ...
  projectManager: {
    ensureRuntime: () => ipcRenderer.invoke('project-manager:ensure-runtime'),
  ...
  ssh: {
    ...
    downloadMany: (workspaceId: string, files: Array<{ path: string; name: string; type: 'file' | 'directory' }>) =>
      ipcRenderer.invoke(IPC_CHANNELS.SSH_DOWNLOAD_MANY, workspaceId, files),
    prepareDrag: (workspaceId: string, files: Array<{ path: string; name: string; type: 'file' | 'directory' }>) =>
