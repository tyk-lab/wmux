...
function projectContractAutonomyPermissions(
  contract: ProjectSupervisorContract,
): SupervisorAutonomyPermission[] {
  return [
    'same-route-next',
    ...(contract.authority.technicalChoices ? ['technical-choice' as const] : []),
    ...(contract.authority.lowRiskRetries ? ['route-adjustment' as const] : []),
...
function queueProjectManagerDelivery(
  ...
  if (sessionId) {
    updatePersistedProjectManagerDeliveries(sessionId, (deliveries) => {
      const persisted = { id: delivery.id, text: delivery.text, createdAt: delivery.createdAt };
...
function queueProjectSupervisorRecovery(lane: SupervisorLane, detail: string): void {
  ...
  queueProjectManagerDelivery([
    ...
    session.status === 'active'
      ? '项目仍处于 active；请核对失败原因后重新执行 supervise。控制层会恢复暂停通道，或在旧监督运行时已失效时重建专属监督。'
      : `项目当前为 ${session.status}；需求版本未变化且没有新的用户问题时，先执行 wmux project resume --project ${session.id}，再重新执行 supervise。`,
    lane.projectWorkItemId
      ? `恢复命令：wmux project supervise --project ${session.id} --task ${lane.projectWorkItemId}`
      : '',
...
async function updateProjectDefinition(
  ...
  if (source === 'user') {
    queueProjectManagerDelivery([
      ...
      '',
      mode === 'pivot'
        ? `旧目标未完成任务已经停止，完成成果仍保留为证据。先执行 wmux project goal-plan --project ${session.id} 建立新目标的 3-7 个阶段目标，再创建新任务；不得把旧任务跨 goalId 复活。`
        : `检查现有任务与调整后的目标是否兼容；继续使用的任务必须通过 task-update 设置 rebindCurrentRequirements=true，过期任务应停止。阶段计划变化时执行 wmux project goal-plan --project ${session.id}。`,
...
async function handleProjectManagerRequest(params: any): Promise<any> {
  ...
  if (['goal-plan', 'task-create', 'task-supervise'].includes(action)) {
  ...
  if (action === 'task-create') {
    if ((params?.workItem || params)?.baseline !== undefined) {
      return { ok: false, error: '项目基线状态由控制层和专属监督维护，项目 AI 不能在任务定义中提供' };
    }
  ...
  if (action === 'task-update') {
    ...
    if (requestedPatch?.workerSurfaceId !== undefined
      ...
      return { ok: false, error: '任务终端和监督通道绑定由控制层维护；项目基线状态由控制层维护，项目管理 AI 不能修改' };
    }
  ...
  if (action === 'task-supervise') {
    ...
    const contractBriefing = [
      ...
      buildProjectSupervisorBriefing({ workItemId, contract: item.contract, baseline: item.baseline }),
    ...
    if (existingLane) {
      if (existingLane.projectWorkItemId === item.id) {
        ...
        if (supervisorLaneControlState(existingLane) === 'paused') {
          store.resumeSupervisorLane(existingLane.id, '项目管理 AI 正在恢复同一任务的专属监督');
          ...
          return {
            ...
            message: '已恢复同一任务的专属监督，并重新投递当前启动或任务协议',
          };
        }
  ...
  w.__wmux_projectManagerRemoteControl = async (params: any) => {
    ...
    if (action === 'start') {
      ...
      deliverProjectManagerMessage([
        ...
        '[首次需求对齐门禁｜必须先执行]',
        PROJECT_MANAGER_ALIGNMENT_GATE,
        `需求充分后，先用 wmux project goal-plan --project ${activeSession.id} --json-file <项目目录内的 .wmux/tmp/文件> 保存 3-7 个粗粒度阶段目标，再 resume 和创建执行任务。阶段目标描述成果、依赖和验收，不得写成命令级微步骤。`,
    ...
    if (action === 'event') {
      ...
      try {
        ...
        console.warn('[project-manager] failed to persist supervisor event before delivery', error);
      }
      scheduleProjectProgressCheck(session.id);
      queueProjectManagerDelivery([
        '[项目监督事件]',
          ...
          if (guard.decision !== 'allow') {
            ...
            if (guard.decision === 'pause') {
              ...
              queueProjectManagerDelivery([
                ...
                `任务：${projectWorkItem.id}`,
                `原因：${guard.reason || '执行护栏已触发'}`,
                '请根据现有证据调整任务合同或路线；不要让监督重复原命令。',
              ].join('\n'), projectSession.id);
            }
            saveProjectManagerSnapshot(projectSession.id);
