...
export function buildProjectSupervisorBriefing(options: {
  ...
  const permissions = [
    ...
    contract.authority.permissionConfirm
      ? `你负责逐次确认命中授权前缀的本地权限请求${contract.authority.targetedTests ? '及定向测试请求' : ''}；项目 AI 只定义合同权限外壳`
      : '不得逐次批准权限请求；先判断它是否为主目标内低风险本地命令，若是则以 contract-change 交项目 AI 收紧前缀并启用 permissionConfirm，只有新增外部访问/凭据/提权/生产或更高风险授权才可能继续上抛用户',
