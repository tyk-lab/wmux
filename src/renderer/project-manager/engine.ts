...
export function projectEffectiveWorkItemPreconditions(
  session: Pick<ProjectManagerSession, 'preconditions'>,
  item: Pick<ProjectWorkItem, 'contract'>,
): string[] {
  return [...new Set([
    ...session.preconditions,
    ...item.contract.preconditions,
  ].map((entry) => entry.trim()).filter(Boolean))];
}

export function buildProjectSupervisorAssignment(
...
export const TASK_VALIDATION_REPORTING_POLICY = [
  ...
  '验证能力缺失或结果无法收敛时，不得自行跳过验收、降低标准或声称完成；同时报告已尝试路线、可行的替代验证、需要人工观察或额外授权的部分。',
].join('');
...
export function normalizeProjectTaskBatch(
  ...
  if (coverage === 'whole-item') {
    ...
    const wholeItemVerification = new Set([
      ...completionDefinition,
      ...evidenceExpectations,
    ].map(projectTaskBatchIdentity));
    const missingValidation = workItem.contract.validation
      .find((criterion) => !wholeItemVerification.has(projectTaskBatchIdentity(criterion)));
    if (missingValidation) {
      return {
        error: `whole-item 必须把工作项全部 validation 放入 completionDefinition 或 evidenceExpectations，当前缺少：${missingValidation}`,
      };
    }
  }
  const taskFields = [
...
export function renderProjectTaskBatch(
  ...
  options: { initializeRepository?: boolean; effectivePreconditions?: readonly string[] } = {},
): string {
  const effectivePreconditions = [...new Set(
    (options.effectivePreconditions || contract.preconditions)
  ...
  return [
    ...
    `本批成果：${batch.outcome}`,
    effectivePreconditions.length > 0 ? `前置条件：\n${effectivePreconditions.map((item) => `- ${item}`).join('\n')}` : '',
    `完成定义：\n${batch.completionDefinition.map((item) => `- ${item}`).join('\n')}`,
    batch.evidenceExpectations.length > 0
      ? `${batch.coverage === 'whole-item'
          ? '验证要求（必须核对并如实报告成功、失败或无法取得）'
...
export function buildProjectSupervisorBriefing(assignment: ProjectSupervisorAssignment): string {
  return [
    `[项目监督工作项] ${assignment.workItemId} · ${assignment.title}`,
    `项目目标：${assignment.projectGoal}`,
    assignment.stage ? `当前阶段：${assignment.stage.id} · ${assignment.stage.title}` : '',
    assignment.stage ? `阶段成果：${assignment.stage.outcome}` : '',
    assignment.stage?.acceptance.length
      ? `阶段验收：${assignment.stage.acceptance.join('；')}`
      : '',
    `工作项成果：${assignment.objective}`,
    assignment.description ? `成果说明：${assignment.description}` : '',
    assignment.effectivePreconditions.length
      ? `有效前置条件：${assignment.effectivePreconditions.join('；')}`
      : '有效前置条件：无',
    assignment.supervisorNotes.length
      ? `监督补充：${assignment.supervisorNotes.join('；')}`
      : '',
    `停止条件：${assignment.stopWhen.join('；')}`,
    `验证要求：${assignment.validation.join('；')}`,
    assignment.stageAcceptanceCoverage.length
      ? '阶段验收映射：' + assignment.stageAcceptanceCoverage.map((mapping) => (
          `${mapping.stageCriterion} ← ${mapping.verificationCriterion}`
        )).join('；')
      : '阶段验收映射：缺失；不得完成派发，先交回项目 AI 补充 canonical coverage。',
    `初始执行模式：${assignment.taskWorkMode}`,
    '验收结果允许成功、失败或当前无法取得，但必须与实际证据一致；失败、未验证或不确定不得声称完成。验证能力缺失且替代路线不能形成新证据时，按稳定角色协议上报情况与可选路线，不得自行跳过或降低验收标准。',
    '验证异常先由监督 AI 在工作项内判断替代验证或返工；超出工作项时上报项目 AI。项目 AI 必须先依据总计划、当前进度和既有授权决策，只有仍无法决定或确需用户专属信息、人工操作、风险授权或验收取舍时，才向用户提交结构化选项。',
  ].filter(Boolean).join('\n');
}
import {
