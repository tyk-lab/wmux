...
export function humanDecisionBoundary(
  ...
  return [
    ...
    projectManaged
      ? '只有需要改变任务契约、跨任务协调、项目级路线调整、健康窗口无法凭新证据续期、硬预算或重试耗尽，或涉及不可逆、高影响及用户专属信息/授权时，才使用 needs-human；控制层会先交给项目管理 AI。项目内取舍仍由项目 AI 决定，只有改变用户目标、对外结果、验收、范围、真实偏好或新增外部访问/风险授权时才继续询问用户。'
