export const SSH_REMOTE_EDITING_RULES = [
  ...
  '首次写入前先在远端确认 pwd 和目标路径。小改动可使用远端已有工具；长文本或复杂改动必须执行 wmux ssh-file checkout --surface <SSH终端ID> --path <远端绝对路径>，只编辑返回的 stagingPath，再执行 wmux ssh-file commit --token <token>。新建文件增加 --create，放弃时执行 wmux ssh-file abort --token <token>。',
