...
兼容 OpenCode、Codex、Kimi、Grok、Pi Agent 等终端 agent；有官方 Hook/扩展事件的自动上报，没有的用 `wmux wrap` 做进程级状态。
...
> 本节只覆盖日常开发与使用；完整参数可通过 `wmux --help` 查看。
...
| Codex CLI | `~/.codex/hooks.json` | 普通终端默认禁用 Hooks；wmux 窗格内按进程启用并保留原生 Hook 信任检查 |
...
成功时终端打印 `[OK] Kimi Code` / `Codex CLI` / … 与路径；有 `[FAIL]` 则退出码非 0。

#### 安装后必做

1. **重启 wmux**（若刚编过 main/renderer）  
2. **重启每个 agent 会话**（kimi / codex / grok / pi），否则仍用旧 hooks
3. Codex：wmux 内启动会启用 Hooks；首次使用或 Hook 内容变化时仍需在 `/hooks` 中审核，wmux 外启动默认不加载 Hooks
...
| **Codex CLI** | `~/.codex/hooks.json` | `codex` | wmux 内临时启用全部已开启 Hook；wmux 外默认禁用 Hook 功能 |
...
| Codex 无 Working | 重启 wmux 与 Codex 终端；确认窗格内的 `codex` 解析到 wmux 自带 shim |
